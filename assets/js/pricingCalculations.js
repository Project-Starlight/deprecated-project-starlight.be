...
function determinePriceWithTwoDiscounts(extraOrBasicOptions, option) {
    ...
    if (allOptionKindOfDiscount === "gratis" || optionKindOfDiscount === "gratis") return "gratis";
        ...
        if (discountData.sortOfMultipleDiscount === "sum") return (option.defaultPrice - (option.defaultPrice * ((option.percentage / 100) + (extraOrBasicOptions.allOptions.percentage / 100))));
        else if (discountData.sortOfMultipleDiscount === "multiply") return (option.defaultPrice * ((1 - (option.percentage / 100)) * (1 - (extraOrBasicOptions.allOptions.percentage / 100))));
